...
const Campaign: React.FC = () => {
  ...
  const handleSubmit = () => {
    const { businessId, name, signupPoints, rewardId, topHeadline, topTitle } =
      campaign;
    if (!businessId) {
  ...
  const processUpdate = () => {
    const { business, reward } = campaign;
    const businessId = business?.id || '';
    const rewardId = reward?.id || '';
    updateMutate({
      id: campaignId,
      campaign: { ...campaign, businessId, rewardId },
    });
  };
                  ...
                  <TableRow key={i}>
                    ...
                    <TableCell>
                      <div className="flex gap-1">
                        ...
                        |
                        <Link
                          href={`/campaign/${uniqueCode ?? ''}`}
                          target="_blank"
                        >
                          <p className="text-blue-800 hover:underline">{`${'localhost:3000'}/campaign/${
                            uniqueCode ?? ''
                          }`}</p>
                        </Link>
                      </div>
