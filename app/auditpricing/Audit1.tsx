...
export const Audit1 = () => {
    ...
    return (
        <motion.div
            ...
            style={{ background: "linear-gradient(to bottom right, #4A90E2, #D6E6F2)" }}
        >
            <p className='flex flex-row items-center text-3xl font-light gap-4'>
                <AiOutlineAudit className='w-8 h-8' /> Audit1
            </p>
            <p className="text-center">You are only eligible for one Audit in only one season</p>
            <p className='text-2xl'> £500 <span className='text-xs'>/budget</span></p>
            <p>20% on First payment</p>
            <p className='flex items-center gap-2'><HiBadgeCheck /> Select just one Audit</p>
            ...
            {selectedSeason ? (
                <p className='text-sm font-medium text-[#000]'>Selected: {selectedSeason}</p>
            ) : (
