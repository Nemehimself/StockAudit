...
import {
  retailQuestionsSpareCapacity,
  retailQuestionsExcessStock,
  retailQuestionsRecommendedSolution,
} from "../Questions/LongForm/RetailQuestions";
...
const Retail = () => {
  const [selectedSpareType, setSelectedSpareType] = useState<
    keyof typeof retailQuestionsSpareCapacity | ""
  >("");
  const [selectedExcessType, setSelectedExcessType] = useState<
    keyof typeof retailQuestionsExcessStock | ""
  >("");
  const [selectedSolutionType, setSelectedSolutionType] = useState<
    keyof typeof retailQuestionsRecommendedSolution | ""
  >("");
  return (
        ...
        <div className="flex flex-col gap-4 h-full">
          <div className="flex flex-row justify-between w-full h-1/3 bg-[#000] bg-opacity-70 p-4 rounded-3xl">
            <div className="w-8/12 h-[24rem] p-4 mr-2 overflow-y-scroll scrollbar-hidden">
              ...
              <select
                ...
                onChange={(e) =>
                  setSelectedSpareType(
                    e.target.value as keyof typeof retailQuestionsSpareCapacity
                  )
                }
              >
                <option value="">Select Retail Type</option>
                {Object.keys(retailQuestionsSpareCapacity).map((type) => (
              ...
              {selectedSpareType &&
                retailQuestionsSpareCapacity[selectedSpareType].map(
                  (question, index) => (
                    <div
                      key={index}
          ...
          <div className="flex flex-row justify-between w-full h-1/3 bg-[#000] bg-opacity-70 p-4 rounded-3xl">
            <div className="w-8/12 h-[24rem] p-4 mr-2 overflow-y-scroll scrollbar-hidden">
              ...
              <select
                ...
                onChange={(e) =>
                  setSelectedExcessType(
                    e.target.value as keyof typeof retailQuestionsExcessStock
                  )
                }
              >
                <option value="">Select Retail Type</option>
                {Object.keys(retailQuestionsExcessStock).map((type) => (
                  <option key={type} value={type}>
                    {type}
                  </option>
                ))}
              ...
              {selectedExcessType &&
                retailQuestionsExcessStock[selectedExcessType].map(
                  (question, index) => (
                    <div
                      key={index}
                      className="flex flex-row justify-between gap-2"
          ...
          <div className="flex flex-row justify-between w-full h-1/3 bg-[#000] bg-opacity-70 p-4 rounded-3xl">
            <div className="w-8/12 h-[24rem] p-4 mr-2 overflow-y-scroll scrollbar-hidden">
              ...
              <select
                ...
                onChange={(e) =>
                  setSelectedSolutionType(
                    e.target
                      .value as keyof typeof retailQuestionsRecommendedSolution
                  )
                }
              >
                <option value="">Select Retail Type</option>
                {Object.keys(retailQuestionsRecommendedSolution).map((type) => (
                  <option key={type} value={type}>
                    {type}
                  </option>
                ))}
              ...
              {selectedSolutionType &&
                retailQuestionsRecommendedSolution[selectedSolutionType].map(
                  (question, index) => (
                    <div
                      key={index}
                      className="flex flex-row justify-between gap-2"
