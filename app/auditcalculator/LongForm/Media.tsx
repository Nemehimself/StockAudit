...
import {
  mediaQuestionsSpareCapacity,
  mediaQuestionsExcessStock,
  mediaQuestionsRecommendedSolution,
} from "../Questions/LongForm/MediaQuestions";
...
const Media = () => {
  const [selectedSpareType, setSelectedSpareType] = useState<
    keyof typeof mediaQuestionsSpareCapacity | ""
  >("");
  const [selectedExcessType, setSelectedExcessType] = useState<
    keyof typeof mediaQuestionsExcessStock | ""
  >("");
  const [selectedSolutionType, setSelectedSolutionType] = useState<
    keyof typeof mediaQuestionsRecommendedSolution | ""
  >("");
  return (
        ...
        <div className="flex flex-col gap-4 h-full">
          <div className="flex flex-row justify-between w-full h-1/3 bg-[#000] bg-opacity-70 p-4 rounded-3xl">
            <div className="w-8/12 h-[24rem] p-4 mr-2 overflow-y-scroll scrollbar-hidden">
              ...
              <select
                ...
                onChange={(e) =>
                  setSelectedSpareType(
                    e.target.value as keyof typeof mediaQuestionsSpareCapacity
                  )
                }
              >
                <option value="">Select Media Type</option>
                {Object.keys(mediaQuestionsSpareCapacity).map((type) => (
              ...
              {selectedSpareType &&
                mediaQuestionsSpareCapacity[selectedSpareType].map(
                  (question, index) => (
                    <div
                      key={index}
          ...
          <div className="flex flex-row justify-between w-full h-1/3 bg-[#000] bg-opacity-70 p-4 rounded-3xl">
            <div className="w-8/12 h-[24rem] p-4 mr-2 overflow-y-scroll scrollbar-hidden">
              ...
              <select
                ...
                onChange={(e) =>
                  setSelectedExcessType(
                    e.target.value as keyof typeof mediaQuestionsExcessStock
                  )
                }
              >
                <option value="">Select Media Type</option>
                {Object.keys(mediaQuestionsExcessStock).map((type) => (
                  <option key={type} value={type}>
                    {type}
                  </option>
                ))}
              ...
              {selectedExcessType &&
                mediaQuestionsExcessStock[selectedExcessType].map(
                  (question, index) => (
                    <div
                      key={index}
                      className="flex flex-row justify-between gap-2"
          ...
          <div className="flex flex-row justify-between w-full h-1/3 bg-[#000] bg-opacity-70 p-4 rounded-3xl">
            <div className="w-8/12 h-[24rem] p-4 mr-2 overflow-y-scroll scrollbar-hidden">
              ...
              <select
                ...
                onChange={(e) =>
                  setSelectedSolutionType(
                    e.target
                      .value as keyof typeof mediaQuestionsRecommendedSolution
                  )
                }
              >
                <option value="">Select Media Type</option>
                {Object.keys(mediaQuestionsRecommendedSolution).map((type) => (
                  <option key={type} value={type}>
                    {type}
                  </option>
                ))}
              ...
              {selectedSolutionType &&
                mediaQuestionsRecommendedSolution[selectedSolutionType].map(
                  (question, index) => (
                    <div
                      key={index}
                      className="flex flex-row justify-between gap-2"
