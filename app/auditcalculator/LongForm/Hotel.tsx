...
import {
  hotelQuestionsSpareCapacity,
  hotelQuestionsExcessStock,
  hotelQuestionsRecommendedSolution,
} from "../Questions/LongForm/HotelQuestions";
...
const Hotel = () => {
  const [selectedSpareType, setSelectedSpareType] = useState<
    keyof typeof hotelQuestionsSpareCapacity | ""
  >("");
        ...
        <div className="flex flex-col gap-4 h-full">
          <div className="flex flex-row justify-between w-full h-1/3 bg-[#000] bg-opacity-70 p-4 rounded-3xl">
            <div className="w-8/12 h-[24rem] p-4 mr-2 overflow-y-scroll scrollbar-hidden">
              ...
              <select
                ...
                onChange={(e) =>
                  setSelectedSpareType(
                    e.target.value as keyof typeof hotelQuestionsSpareCapacity
                  )
                }
              >
                <option value="">Select Hotel Type</option>
                {Object.keys(hotelQuestionsSpareCapacity).map((type) => (
              ...
              {selectedSpareType &&
                hotelQuestionsSpareCapacity[selectedSpareType].map(
                  (question, index) => (
                    <div
                      key={index}
          ...
          <div className="flex flex-row justify-between w-full h-1/3 bg-[#000] bg-opacity-70 p-4 rounded-3xl">
            <div className="w-8/12 h-[24rem] p-4 mr-2 overflow-y-scroll scrollbar-hidden">
              ...
              <select
                ...
                onChange={(e) =>
                  setSelectedExcessType(
                    e.target.value as keyof typeof hotelQuestionsExcessStock
                  )
                }
              >
                <option value="">Select Hotel Type</option>
                {Object.keys(hotelQuestionsExcessStock).map((type) => (
                  <option key={type} value={type}>
                    {type}
                  </option>
                ))}
              ...
              {selectedExcessType &&
                hotelQuestionsExcessStock[selectedExcessType].map(
                  (question, index) => (
                    <div
                      key={index}
                      className="flex flex-row justify-between gap-2"
          ...
          <div className="flex flex-row justify-between w-full h-1/3 bg-[#000] bg-opacity-70 p-4 rounded-3xl">
            <div className="w-8/12 h-[24rem] p-4 mr-2 overflow-y-scroll scrollbar-hidden">
              ...
              >
                <option value="">Select Hotel Type</option>
                {Object.keys(hotelQuestionsRecommendedSolution).map((type) => (
                  <option key={type} value={type}>
                    {type}
                  </option>
                ))}
              ...
              {selectedSolutionType &&
                hotelQuestionsRecommendedSolution[selectedSolutionType].map(
                  (question, index) => (
                    <div
                      key={index}
                      className="flex flex-row justify-between gap-2"
