...
import {mediaQuestionsRecommendedSolution} from "../Questions/LongForm/MediaQuestions";
...
type MediaGroups = keyof typeof MediaSpareQuestions;

const Media : React.FC<MediaProps & { selectedGroup: MediaGroups }> = ({ selectedGroup }) => {
  ...
  if (!groupData) {
    return <div className="text-white">Please select a Media group.</div>;
  }
  ...
  return (
    <div className="flex flex-col justify-center items-center  gap-4 w-full p-6">
      ...
      <div className="flex flex-col justify-center w-full px-4 bg-cover bg-center bg-no-repeat  h-full">
        ...
        {Object.entries(mediaExcessStock).map(([key, data]) => (
          ...
          <div className="flex flex-row justify-between w-2/3 h-1/3 bg-[#000] bg-opacity-70 p-4 rounded-3xl">
            ...
            <div className="w-3/4 h-[24rem] p-4 mr-2 overflow-y-scroll scrollbar-hidden">
              ...
              >
                <option value="">Select Media Type</option>
                {Object.keys(mediaQuestionsRecommendedSolution).map(
                  (type) => (
